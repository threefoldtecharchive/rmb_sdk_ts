...
class Client {
  ...
  createConnection() {
    try {
      ...
      throw new Error({
        message: `Unable to create websocket connection due to ${err}`,
      });
    }
  }
  async connect() {
  ...
  async send(
    ...
    try {
      ...
      let retriesCount = 0;
      while (
        this.con.readyState != this.con.OPEN &&
  ...
  read(requestID: string) {
    return new Promise(async (resolve, reject) => {
      let envelope = this.responses.get(requestID)!;
      // check if envelope in map has a response
      const now = new Date().getTime();
      while (
        envelope &&
        new Date().getTime() < now + envelope.expiration * 1000
      ) {
        ...
        if (envelope && envelope.response) {
          ...
          if (verified) {
            ...
            if (dataReceived) {
              const decodedData = new TextDecoder("utf8").decode(
                Buffer.from(dataReceived)
              );
